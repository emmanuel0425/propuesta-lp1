...
namespace Capa_Negocio.Logica
{
    class CActualizar
    {
        public void Update(CEmpleado objEmpleado, ref bool ResultadoOk, ref string MensajeError)
        {
            ...
            try
            {
                ...
                if (objEmpleado.Empleado_ID != 0)
                {
                    ...
                    objEmpleado.Empleado_ID = Convert.ToInt32(datatable.Rows[15]["Empleado_ID"]);
                }
            ...
            {
                ResultadoOk = false;
                MensajeError = "Ha ocurrido un error al actualizar los datos del empleado. " + error.Message.ToString();
            }
            finally
            {
                con.Close();
